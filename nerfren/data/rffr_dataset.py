...
class RFFRDataset(BaseDataset):
    ...
    def read_meta(self):
        ...
        if self.opt.model == "nerf":
            self.mask_paths = [os.path.join(self.root_dir, 'refl_masks', name.split('.')[0] + '.png') for name in self.image_names]
        else:
            self.mask_paths = [os.path.join(self.root_dir, 'refl_masks'+self.opt.mask_gen, name.split('.')[0] + '.png')
                                for name in self.image_names]
        ...
        if self.split == 'train': # create buffer of all rays and rgb data
            ...
            for i, image_path in enumerate(self.image_paths):
                ...
                if i not in self.train_idxs: # exclude the val images
                    #print(f'excluding {i}, {image_path}')
                    continue
                ...
                if os.path.exists(self.mask_paths[i]):
                    temp = cv2.imread(self.mask_paths[i], cv2.IMREAD_UNCHANGED)
                    #print(len(temp.shape))
                    #if there isnt a field (shape[2])for channels, the image is grayscale (only 1 channel)
                    is_grayscale = (len(temp.shape) == 2)
                    if is_grayscale:
                        mask = temp
                        #mask[:,:] = mask[:,:]/255
                    ...
                    # To see reflection mask result
                    #windowname = f"{self.mask_paths[i][-12:]}"
                    #cv2.namedWindow(windowname)
                    #cv2.moveWindow(windowname, 40, 30)
                    #check_image = cv2.imshow(windowname, mask)
                    #cv2.waitKey(0)     

                    mask = torch.from_numpy(mask).view(-1, 1)
                    
                    #np.savetxt(f"{self.mask_paths[i][-12:-4]}.txt",mask)
                    #np.savetxt(f"torch.ones_like{self.mask_paths[i][-12:-4]}.txt",torch.ones_like(mask))
                    self.all_masks += [mask]                

                    if self.opt.num_masks == -1 or count < self.opt.num_masks:
                        self.all_masks_valid += [torch.ones_like(mask)]
                        print(f"Adding {self.mask_paths[i]} to all_masks_valid")
        ...
        if self.split == 'train': # use data in the buffers
            ...
            if self.split in ['val', 'test_train', 'test_val']:
                ...
                if os.path.exists(self.mask_paths[idx]):
                    temp = cv2.imread(self.mask_paths[idx], cv2.IMREAD_UNCHANGED)
                    is_grayscale = (len(temp.shape) == 2)
                    if is_grayscale:
                        mask = temp
                    else:
