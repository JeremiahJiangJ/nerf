...
def train(Dataset, Network, cfg):
    ...
    loader = DataLoader(data, collate_fn=data.collate, batch_size=cfg.batch, shuffle=True, num_workers=0)# IF NUM_WORKERS != 0, there will be NoneType error in enumerate

    '''
    Debugging NoneType is not callable
    ...
    '''
    ## val dataloader
    val_cfg = Dataset.Config(datapath='D:/HetNet/datasets/MSD', mode='test')
    ...
    for epoch in range(cfg.epoch):
        ...
        for step, (image, mask, edge) in enumerate(loader):
            ...
            if step % 10 == 0:
                print('%.05f | step:%d/%d/%d | lr=%.6f | loss=%.6f' % (time.time() - step_start, global_step, epoch+1, cfg.epoch, optimizer.param_groups[1]['lr'], loss.item()))
...
if __name__ == '__main__':
    ...
    cfg = dataset.Config(dataset='MSD', datapath='D:/HetNet/datasets/MSD', savepath=f'./{EXP_NAME}/', mode='train', batch=12, lr=0.01, momen=0.9, decay=5e-4, epoch=150)
